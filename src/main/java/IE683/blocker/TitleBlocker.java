...
public class TitleBlocker extends RecordBlockingKeyGenerator<Movie, Attribute> {
    ...
    public void generateBlockingKeys(Movie record, Processable<Correspondence<Attribute, Matchable>> correspondences,
        ...
        for (int i = 0; i <= 2 && i < tokens.length; i++) {
            blockingKeyValue.append(tokens[i].substring(0, Math.min(2, tokens[i].length())).toUpperCase());
        }
